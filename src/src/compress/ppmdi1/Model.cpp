...
#pragma warning (push)
#pragma warning (disable: 4018 4244 4389 4146 )
...
int _STDCALL DecodeFile(_PPMD_FILE* DecodedFile,_PPMD_FILE* EncodedFile,
                            int MaxOrder,MR_METHOD MRMethod)
{
    ...
    for (BYTE ns=MinContext->NumStats; ; ) {
        ...
        //如果取消
        if (DecodedFile->m_IsCanceled || DecodedFile->m_IsCanceled)
        {
            break;
        }
