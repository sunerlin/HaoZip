...
void LzmaEnc_FastPosInit(byte_t *g_FastPos)
{
  int c = 2, slotFast;
  g_FastPos[0] = 0;
  g_FastPos[1] = 1;
...
typedef struct
{
  ...
  #ifndef LZMA_LOG_BSR
  byte_t g_FastPos[1 << kNumLogBits];
  #endif
...
result_t LzmaEnc_SetProps(CLzmaEncHandle pp, const CLzmaEncProps *props2)
{
  CLzmaEnc *p = (CLzmaEnc *)pp;
  CLzmaEncProps props = *props2;
  LzmaEncProps_Normalize(&props);
...
static int RangeEnc_Alloc(CRangeEnc *p, ISzAlloc *alloc)
{
  if (p->bufBase == 0)
  {
    p->bufBase = (byte_t *)alloc->Alloc(alloc, RC_BUF_SIZE);
    if (p->bufBase == 0)
      return 0;
...
static void MY_FAST_CALL RangeEnc_ShiftLow(CRangeEnc *p)
{
  if ((uint32_t)p->low < (uint32_t)0xFF000000 || (int)(p->low >> 32) != 0)
  {
    ...
    {
      byte_t *buf = p->buf;
      *buf++ = (byte_t)(temp + (byte_t)(p->low >> 32));
      p->buf = buf;
      if (buf == p->bufLim)
    ...
    while (--p->cacheSize != 0);
    p->cache = (byte_t)((uint32_t)p->low >> 24);
  }
  p->cacheSize++;
...
static uint32_t ReadMatchDistances(CLzmaEnc *p, uint32_t *numDistancePairsRes)
{
  ...
  if (numPairs > 0)
  {
    ...
    {
      const byte_t *pby = p->matchFinder.GetPointerToCurrentPos(p->matchFinderObj) - 1;
      uint32_t distance = p->matches[numPairs - 1] + 1;
      uint32_t numAvail = p->numAvail;
      if (numAvail > LZMA_MATCH_LEN_MAX)
        numAvail = LZMA_MATCH_LEN_MAX;
      {
        const byte_t *pby2 = pby - distance;
...
static uint32_t GetOptimum(CLzmaEnc *p, uint32_t position, uint32_t *backRes)
{
  ...
  for (;;)
  {
    ...
    if (!nextIsChar && matchByte != curByte) /* speed optimization */
    {
      ...
      uint32_t temp;
      uint32_t lenTest2;
      const byte_t *data2 = data - (reps[0] + 1);
    ...
    for (repIndex = 0; repIndex < LZMA_NUM_REPS; repIndex++)
    {
      ...
      uint32_t price;
      const byte_t *data2 = data - (reps[repIndex] + 1);
      if (data[0] != data2[0] || data[1] != data2[1])
        continue;
      for (lenTest = 2; lenTest < numAvail && data[lenTest] == data2[lenTest]; lenTest++);
    ...
    if (newLen >= startLen)
    {
      ...
      for (lenTest = /*2*/ startLen; ; lenTest++)
      {
        ...
        if (/*_maxMode && */lenTest == matches[offs])
        {
          ...
          const byte_t *data2 = data - (curBack + 1);
          uint32_t lenTest2 = lenTest + 1;
          uint32_t limit = lenTest2 + p->numFastBytes;
          uint32_t nextRepMatchPrice;
          if (limit > numAvailFull)
...
static uint32_t GetOptimumFast(CLzmaEnc *p, uint32_t *backRes)
{
  ...
  *backRes = (uint32_t)-1;
  ...
  {
    uint32_t len;
    const byte_t *data2 = data - (p->reps[i] + 1);
    if (data[0] != data2[0] || data[1] != data2[1])
      continue;
...
static result_t CheckErrors(CLzmaEnc *p)
{
  if (p->result != SZ_OK)
    return p->result;
  if (p->rc.res != SZ_OK)
...
static result_t Flush(CLzmaEnc *p, uint32_t nowPos)
{
  /* ReleaseMFStream(); */
  p->finished = True;
  if (p->writeEndMark)
...
static result_t LzmaEnc_CodeOneBlock(CLzmaEnc *p, Bool useLimits, uint32_t maxPackSize, uint32_t maxUnpackSize)
{
...
static result_t LzmaEnc_Alloc(CLzmaEnc *p, uint32_t keepWindowSize, ISzAlloc *alloc, ISzAlloc *allocBig)
{
...
static result_t LzmaEnc_AllocAndInit(CLzmaEnc *p, uint32_t keepWindowSize, ISzAlloc *alloc, ISzAlloc *allocBig)
{
  uint32_t i;
...
static result_t LzmaEnc_Prepare(CLzmaEncHandle pp, ISeqOutStream *outStream, ISeqInStream *inStream,
...
result_t LzmaEnc_PrepareForLzma2(CLzmaEncHandle pp,
...
static void LzmaEnc_SetInputBuf(CLzmaEnc *p, const byte_t *src, size_t srcLen)
{
  p->matchFinderBase.directInput = 1;
  p->matchFinderBase.bufferBase = (byte_t *)src;
...
static result_t LzmaEnc_Encode2(CLzmaEnc *p, ICompressProgress *progress)
{
  result_t res = SZ_OK;

  #ifdef COMPRESS_MF_MT
  byte_t allocaDummy[0x300];
  int i = 0;
  for (i = 0; i < 16; i++)
...
result_t LzmaEnc_Encode(CLzmaEncHandle pp, ISeqOutStream *outStream, ISeqInStream *inStream, ICompressProgress *progress,
    ISzAlloc *alloc, ISzAlloc *allocBig)
{
...
result_t LzmaEnc_WriteProperties(CLzmaEncHandle pp, byte_t *props, size_t *size)
{
  CLzmaEnc *p = (CLzmaEnc *)pp;
  int i;
  ...
  *size = LZMA_PROPS_SIZE;
  props[0] = (byte_t)((p->pb * 5 + p->lp) * 9 + p->lc);
  ...
  for (i = 0; i < 4; i++)
    props[1 + i] = (byte_t)(dictSize >> (8 * i));
  return SZ_OK;
}

result_t LzmaEnc_MemEncode(CLzmaEncHandle pp, byte_t *dest, size_t *destLen, const byte_t *src, size_t srcLen,
