...
static int LzInWindow_Create(CMatchFinder *p, uint32_t keepSizeReserv, ISzAlloc *alloc)
{
  ...
  if (p->bufferBase == 0 || p->blockSize != blockSize)
  {
    ...
    p->blockSize = blockSize;
    p->bufferBase = (byte_t *)alloc->Alloc(alloc, (size_t)blockSize);
  }
  return (p->bufferBase != 0);
}

byte_t *MatchFinder_GetPointerToCurrentPos(CMatchFinder *p) { return p->buffer; }
byte_t MatchFinder_GetIndexByte(CMatchFinder *p, int32_t index) { return p->buffer[index]; }
...
static void MatchFinder_ReadBlock(CMatchFinder *p)
{
  ...
  for (;;)
  {
    byte_t *dest = p->buffer + (p->streamPos - p->pos);
...
static uint32_t * Hc_GetMatchesSpec(uint32_t lenLimit, uint32_t curMatch, uint32_t pos, const byte_t *cur, CLzRef *son,
...
uint32_t * GetMatchesSpec1(uint32_t lenLimit, uint32_t curMatch, uint32_t pos, const byte_t *cur, CLzRef *son,
    uint32_t _cyclicBufferPos, uint32_t _cyclicBufferSize, uint32_t cutValue,
    uint32_t *distances, uint32_t maxLen)
{
...
static void SkipMatchesSpec(uint32_t lenLimit, uint32_t curMatch, uint32_t pos, const byte_t *cur, CLzRef *son,
    uint32_t _cyclicBufferPos, uint32_t _cyclicBufferSize, uint32_t cutValue)
{
  CLzRef *ptr0 = son + (_cyclicBufferPos << 1) + 1;
  CLzRef *ptr1 = son + (_cyclicBufferPos << 1);
...
#define GET_MATCHES_HEADER2(minLen, ret_op) \
  uint32_t lenLimit; uint32_t hashValue; const byte_t *cur; uint32_t curMatch; \
