...
#define UPDATE_PROGRESS(size, prev, total) \
    if (size != (uint64_t)(int64_t)-1) { total += size - prev; prev = size; }
...
static void MtProgress_SetError(CMtProgress *p, result_t res)
{
    CriticalSection_Enter(&p->cs);
    if (p->res == SZ_OK)
        p->res = res;
...
static void MtCoder_SetError(CMtCoder* p, result_t res)
{
    CriticalSection_Enter(&p->cs);
    if (p->res == SZ_OK)
        p->res = res;
...
static result_t CMtThread_Prepare(CMtThread *p)
{
...
static result_t FullRead(ISeqInStream *stream, byte_t *data, size_t *processedSize)
{
    ...
    *processedSize = 0;
    ...
    {
        size_t curSize = size;
        result_t res = stream->Read(stream, data, &curSize);
...
static result_t MtThread_Process(CMtThread *p, Bool *stop)
{
    CMtThread *next;
    *stop = True;
...
static THREAD_FUNC_RET_TYPE THREAD_FUNC_CALL_TYPE ThreadFunc(void *pp)
{
    ...
    for (;;)
    {
        ...
        result_t res = MtThread_Process(p, &stop);
        if (res != SZ_OK)
        {
