...
#else
            if (m_OpenCallback && m_CallbackData)
            {
                if (!m_OpenCallback(m_CallbackData, Cmd->Password, sizeof(Cmd->Password)))
                {
                    Close();
                    ErrHandler.Exit(USER_BREAK);
                }
            }
            else if (!GetPassword(PASSWORD_ARCHIVE,FileName,Cmd->Password,sizeof(Cmd->Password)))
            {
                Close();
                ErrHandler.Exit(USER_BREAK);
            }
