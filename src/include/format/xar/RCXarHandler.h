...
#ifndef __RCXarHandler_h_
#define __RCXarHandler_h_ 1
...
private:
    ...
    */
    static bool AddItem(const RCXmlItem& item, RCVector<RCXarFile>& files, int32_t parent) ;
    ...
    @return 成功返回true,否则返回false
    */
    static bool ParseUInt64(const RCXmlItem& item, const char* name, uint64_t& res) ; 
    ...
    @return 返回解析后时间
    */
    static uint64_t ParseTime(const RCXmlItem& item, const char* name) ;
    ...
    @return 成功返回true,否则返回false
    */
    static bool ParseNumber(const char* s, int32_t size, uint32_t& res) ; 

    /** 解析sha1
    @param [in] item xml项
    @param [in] name 名称
    @param [out] digest 返回hash值
    @return 成功返回true,否则返回false
    */
    static bool ParseSha1(const RCXmlItem& item, const char* name, byte_t* digest) ; 
    ...
    @return 成功返回true,否则返回false
    */
    static bool HexToByte(char c, byte_t& res) ; 
...
private:
    ...
    */
    ...
    /** 文件列表
    */
    RCVector<RCXarFile> m_files ;
};

END_NAMESPACE_RCZIP

#endif //__RCXarHandler_h_
